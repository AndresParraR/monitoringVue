  ...
  <nav>
    ...
    >
      <v-img
        ...
        <v-divider></v-divider>
        {{$store.state.colour}}
        <v-list-item v-for="link in $store.state.links" :key="link.title" router :to="link.route" @click="$store.dispatch('CLICK_LINK', link); clickLink()" class="mt-7" style="border-radius: .25rem; color: #FFF !important;" :style="link.isFocused && $store.state.styleFocused">       
          <v-list-item-action >
